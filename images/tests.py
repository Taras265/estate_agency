...
class ImagesTest(TestCase):
    ...
    def test_apartment_list_failure(self):
        response = self.client.get(reverse_lazy('images:apartment_images_list',
                                                kwargs={'lang': 'en',
                                                        'pk': self.apartment.id}))
    ...
    def test_create_apartment_failure(self):
        response = self.client.get(reverse_lazy('images:apartment_image_add',
                                                kwargs={'lang': 'en',
                                                        'pk': self.apartment.id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_handbook_failure(self):
        response = self.client.get(reverse_lazy('images:apartment_image_delete',
                                                kwargs={'lang': 'en',
                                                        'pk': self.apartment.id}))
        self.assertEqual(response.status_code, 302)
